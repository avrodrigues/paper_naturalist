...
for(i in seq_along(species.names)){
  ...
  env.df.spec <- as.data.frame(extract(env.stack, points.spec)) %>% 
    setNames(c("altitude", "temp_mean", "prec_annual",
               "prec_season", "prec_dry_quarter", "temp_season")) 
  
  env.df.not.spec <- as.data.frame(extract(env.stack, points.not.spec)) %>% 
    setNames(c("altitude", "temp_mean", "prec_annual",
               "prec_season", "prec_dry_quarter", "temp_season")) 
...
saveRDS(permanova.list, "output/permanova_list.rds")
